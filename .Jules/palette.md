...
**Learning:** Elements positioned flush against the viewport edges (like a sticky "Skip to main content" link at `top: 0; left: 0;`) will have their default focus rings clipped by the browser window, making them invisible or hard to see for keyboard users.
**Action:** Always use a negative `outline-offset` (e.g., `-2px` or `-4px`) on `:focus-visible` for such elements to ensure the focus ring draws completely inside the element's bounding box and remains fully visible.
